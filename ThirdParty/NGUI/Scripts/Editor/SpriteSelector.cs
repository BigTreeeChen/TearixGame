...
public class SpriteSelector : ScriptableWizard
{
	...
	void OnGUI ()
	{
		...
		else
		{
            ...
            if (texPath.EndsWith(".rgb.png"))
            {
                texPath = texPath.Replace(".rgb", "");
            }
            Texture2D texShown = AssetDatabase.LoadAssetAtPath(texPath, typeof(Texture2D)) as Texture2D;
				...
				GUILayout.BeginHorizontal();
				{
					...
					for (; offset < sprites.size; ++offset)
					{
						...
						if (Event.current.type == EventType.Repaint)
						{
							...
							GUI.DrawTextureWithTexCoords(clipRect, texShown, uv);
