...
public class T4MSC : EditorWindow {
	...
	void Optimize(){
		...
		if (GUILayout.Button("Process", GUILayout.Width(100), GUILayout.Height(30))) 
		{
			...
			if (countchild>0){
				...
				SGameEngine.SGameLog.log(AssetName);
	...
	void DeactivateBillBByScript(){
		...
		SGameEngine.SGameLog.log_error ("The Number of Activated Billboard Objects has changed, reactivate the billboards in the 'Billboard' Tab.");
	}
	...
	void ActivateDeactivateBillBoard(){
			if (billActivate == true){ //si le billboard est actif
					...
					PrefabUtility.RecordPrefabInstancePropertyModifications(CurrentSelect.gameObject.GetComponent<T4MObjSC>());
					billActivate = false;
					SGameEngine.SGameLog.log_error ("Billboard deactivated !");
			}else{
					if (!PlayerCam)
					...
					billActivate = true;
					SGameEngine.SGameLog.log_error ("Billboard (re)activated !");
			}	
	}
	...
	void DeactivateLODByScript(){
		...
		PrefabUtility.RecordPrefabInstancePropertyModifications(CurrentSelect.gameObject.GetComponent<T4MObjSC>());
		SGameEngine.SGameLog.log ("The Number of Activated LOD Objects has changed, reactivate the billboards in the 'LOD' Tab.");
	...
	void ActivateDeactivateLOD (){
		if (LodActivate == true){ //Lod actif
					...
					PrefabUtility.RecordPrefabInstancePropertyModifications(CurrentSelect.gameObject.GetComponent<T4MObjSC>());
					LodActivate = false;
					SGameEngine.SGameLog.log_error ("LOD deactivated !");
				}else{
					if (!PlayerCam)
					PlayerCam=Camera.main.transform;
				...
				LodActivate = true;
				SGameEngine.SGameLog.log_error ("LOD (re)activated !");
			}
		CurrentSelect.gameObject.GetComponent <T4MObjSC>().Awake();
	}
		...
		if (ShaderModel == SM.ShaderModel1){
			...
			if (MenuTextureSM1 == EnumShaderGLES1.T4M_2_Textures_ManualAdd_CustoLM_1DC){	
				...
				//Diffuse SM2
					...
					CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.shader =  Shader.Find("Inception-Scene/T4MTwoTextures");
				}else if (MenuTextureSM2 == EnumShaderGLES2.T4M_3_Textures_HighSpec){							
					CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.shader =  Shader.Find("Inception-Scene/T4MThreeTextures");						
				}else if (MenuTextureSM2 == EnumShaderGLES2.T4M_4_Textures_HighSpec){							
					CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.shader =  Shader.Find("Inception-Scene/T4MFourTextures");		
				}else if (MenuTextureSM2 == EnumShaderGLES2.T4M_4_Textures_HighSpec_CUBE){							
					CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.shader =  Shader.Find("Inception-Scene/T4MFourTexturesCube");		
	 ...
	 void ConvertUTerrain()
	{
        ...
        try {
            ...
            SGameEngine.SGameLog.log("Error saving file: " + err.Message);
        }
        sw.Close();
		AssetDatabase.SaveAssets(); 
		...
		Material Tmaterial;
		Tmaterial = new Material (Shader.Find("Inception-Scene/T4MFourTextures"));
	...
	void Obj2T4M()
	{
		...
		AssetDatabase.ImportAsset (path, ImportAssetOptions.ForceUpdate);
		Material Tmaterial;
		Tmaterial = new Material (Shader.Find("Inception-Scene/T4MThreeTextures"));
		AssetDatabase.CreateAsset(Tmaterial, T4MPrefabFolder+"Terrains/Material/"+FinalExpName+".mat");
		AssetDatabase.ImportAsset (T4MPrefabFolder+"Terrains/Material/"+FinalExpName+".mat", ImportAssetOptions.ForceUpdate);
	...
	void CheckShader(){
		...
		if (CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.shader == Shader.Find("Inception-Scene/T4MTwoTextures")){
			MenuTextureSM2 = EnumShaderGLES2.T4M_2_Textures_HighSpec;
			ShaderModel = SM.ShaderModel2;
		}else
		if (CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.shader == Shader.Find("Inception-Scene/T4MThreeTextures")){
			MenuTextureSM2 = EnumShaderGLES2.T4M_3_Textures_HighSpec;
			ShaderModel = SM.ShaderModel2;
		}else
		if (CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.shader == Shader.Find("Inception-Scene/T4MFourTextures")){
			MenuTextureSM2 = EnumShaderGLES2.T4M_4_Textures_HighSpec;
			ShaderModel = SM.ShaderModel2;
		}else
		if (CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.shader == Shader.Find("Inception-Scene/T4MFourTexturesCube")){
			MenuTextureSM2 = EnumShaderGLES2.T4M_4_Textures_HighSpec_CUBE;
			ShaderModel = SM.ShaderModel2;
		}else
